...
function Chip({ ok, textos }) {
  ...
  return (
    <span style={{ fontFamily: F.display, fontWeight: 700, fontSize: 10.5, letterSpacing: "0.06em", padding: "3px 9px", borderRadius: 20, background: ok ? C.ok : C.nok, color: "#fff", whiteSpace: "nowrap" }}>
      {ok ? sim : nao}
    </span>
  );
}
...
export default function App() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: F.body, color: C.texto }}>
      ...
      {/* ── CABEÇALHO ── */}
      <header className="no-print" style={{ background: C.navy }}>
        <div style={{ maxWidth: 880, margin: "0 auto", padding: "12px 16px", display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div className="print-area" style={{ maxWidth: 800, margin: "0 auto", background: "#fff", padding: "30px 36px", boxShadow: "0 2px 18px rgba(20,30,80,0.14)", borderRadius: 4 }}>
            ...
            {relTipo === "diario" && (
              <>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  <thead><tr>{["Nº", "Hora", "Placa", "Temp. (°C)", "Massa (t)", "Destino", "Situação"].map((h) => <th key={h} style={th}>{h}</th>)}</tr></thead>
                  <tbody>
                    {cargas.map((c) => (
                      <tr key={c.id}>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: 8 }}>
                  <thead><tr>{["Ensaio", "Projeto", "Tolerância", "Resultado", "Situação"].map((h) => <th key={h} style={th}>{h}</th>)}</tr></thead>
                  <tbody><tr>
                    <td style={{ ...td, textAlign: "left", fontFamily: F.body }}>Teor de ligante — {lab.metodo}</td>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  <thead><tr>{["Peneira", "Norma", "Projeto", "Limite aplicado", "Medido", "Situação"].map((h) => <th key={h} style={th}>{h}</th>)}</tr></thead>
                  <tbody>
                    {peneirasF.map((p) => {
            ...
            {relTipo !== "diario" && cargaSel && (
              <>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  <thead><tr>{["Especificação", "Medido", "Massa (t)", "Situação"].map((h) => <th key={h} style={th}>{h}</th>)}</tr></thead>
                  <tbody><tr>
